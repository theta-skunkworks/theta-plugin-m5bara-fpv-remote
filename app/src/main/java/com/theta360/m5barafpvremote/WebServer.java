...
public class WebServer extends NanoHTTPD {
    ...
    private Response servePreviewCommnand(Method method, String uri, String strJson) {
        ...
        try {
            ...
            } else if ( name.equals("camera.baraControl") ) {
                JSONObject parameters = command.getJSONObject("parameters");
                int rcvCmdNo;
                if (parameters.isNull("commandNo")) {
                    rcvCmdNo = 0;
                } else {
                    rcvCmdNo = parameters.getInt("commandNo");
                }
                Log.d(TAG, "camera.baraControl : commandNo=" + String.valueOf(rcvCmdNo));
                mCallback.execCtrlBala(rcvCmdNo);
                return newFixedLengthResponse(Response.Status.OK, "text/plain", "{\"name\":\"camera.baraControl\", \"state\":\"done\"}");
